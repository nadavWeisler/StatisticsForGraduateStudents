setwd("D:/OneDrive/School/מעשית מחקרית/TwoSideRmsExtention-JS/Result csvs/")
install.packages()
data=read.csv(file='Data for R.csv')
data2=read.csv(file='Less Data for R.csv')
head(data)
str(data)
head(data2)
str(data2)
#data[data$Chose_priority==0,]$Chose_priority<-"No"
#data[data$Chose_priority==1,]$Chose_priority<-"Yes"
#data$Chose_priority <- as.factor(data$Chose_priority)
...
#no extra values
prio<-glmer(Chose_priority~1+(1|Participant_number),data,family=binomial)
summary(prio)
exp(confint(prio))
sum=data %>% group_by(Participant_number) %>% summarise(p_dominant=mean(Chose_priority))
hist(sum$p_dominant, main="Ratio of Choosing Priority Stimulus Across Participants", xlab="Ratio of Choosing Priority Stimulus", ylab="Frequency Across Participants")

#no extra values filtered
prio2<-glmer(Chose_priority~1+(1|Participant_number),data2,family=binomial)
summary(prio2)
exp(confint(prio2))
sum=data2 %>% group_by(Participant_number) %>% summarise(p_dominant2=mean(Chose_priority))
hist(sum$p_dominant2, main="Ratio of Choosing Priority Stimulus Across Participants", xlab="Ratio of Choosing Priority Stimulus", ylab="Frequency Across Participants")


#interaction
inter<-glmer(Chose_priority~scale(Priority_difference)*scale(dominant_priority)+(1|Participant_number),data,family=binomial)
summary(inter)
...
#dif
predicted.data <-data.frame(probability.of.pr=logistic$fitted.values, pr=data$Chose_priority, dif=data$Priority_difference)
predicted.data <- predicted.data[order(predicted.data$probability.of.pr, decreasing=FALSE),]
ggplot(data=predicted.data, aes(x=dif, y=probability.of.pr)) + geom_point(aes(color=pr), alpha=1, shape=4, stroke=2) + xlab("Priority Difference") + ylab("predicted probability to choose priority")
